...
func (h *AdminHandler) ListWallpapers(w http.ResponseWriter, r *http.Request) {
	...
	rows, total, err := h.wallpaperRepo.AdminList(r.Context(), repo.AdminWallpaperListOpts{
		...
		UserID:        userID,
		WallpaperType: q.Get("wallpaper_type"),
...
func (h *AdminHandler) hardDeleteOne(ctx context.Context, id int64) error {
	...
	// touched: the original upload, the three poster tiers, any legacy
	// preview video, dynamic
	...
	objectURLs := []string{wp.OriginalURL, wp.ThumbURL, wp.PreviewURL, wp.PosterURL, wp.PreviewVideoURL}
	if wp.FrameURLs != "" {
