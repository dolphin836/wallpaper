...
# NOTE: keep the env assignments and `go run` on one continued command.
# A comment line between a trailing `\` and the command silently breaks
# the continuation, so the env (DB_PORT etc.) never reaches `go run` and
# it falls back to localhost:5432 → connection refused. (The
# ${arr[@]+"${arr[@]}"} form expands to nothing when the array is empty
# without tripping `set -u` on macOS's bash 3.2, where a plain
# "${arr[@]}" would error.)
DB_HOST=127.0.0.1 \
DB_PORT="${TUNNEL_PORT}" \
DB_USER=wallpaper \
